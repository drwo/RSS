...
This section installs some selected R packages into a site-wide library. You should be logged into RSS as user rstudio and have uploaded thisnote book using the Files tab.

## 3.1 Setting Up for System-Wide Installation
This section sets up RSS so that user rstudio can install packages into a site-wide library making any pacakges there available to all users. The following chunk displays the site-wide libraries that are available:
...
The first library directory listed is exclusive to user rstudio. We are going to modify /usr/local/lib/R/site-library so that user rstudio can install pacakges there.
```{bash}
...
Before installing these R pacakges we install the following Linux libraries that are required for knitting to PDFs.
...
MySQL as an open source relational database.  Here we install MySQL and an administrator accounts with full privileges.
...
The default installation of MySQL only allows logging in locally. To enable remote log in you must edit the file /etc/mysql/mysql.conf.d/mysqld.cnf changing the value of the variable "bind-address" from 127.0.0.1 to 0.0.0.0. We do not cover that here.

## 4.2 Package RMySQL
The R package RMySQL is needed to access the MySQL server from an R program. We also need another Linux library. Yous should still be logged into the VM as root. If not do so.
```{bash}
